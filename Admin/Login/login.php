...
<body>
    ...
    $query = "SELECT user_id, approved FROM hospital_staff WHERE email = ?";
    ...
    if ($result->num_rows > 0) {
        // Email exists, fetch user_id and approval status
        $row = $result->fetch_assoc();
        $user_id = $row['user_id'];
        $approved = $row['approved'];

        // If approved, store session variables and redirect to the dashboard
        if ($approved) {
            ...
            echo '<script>alert("Your account is awaiting approval.");</script>';
        }
    } else {
        // If email does not exist, show a message
